...
echo "[entrypoint] starting: /code/main.py" >&2
/code/main.py &
CHILD_PID=$!
wait "$CHILD_PID"
code=$?
...
if [ "$STOP" -eq 0 ]; then
    echo "[entrypoint] not restarting automatically -- container staying alive; exec in and run '/code/main.py [flags]' manually" >&2
    while [ "$STOP" -eq 0 ]; do
        sleep 3600 &
        CHILD_PID=$!
        wait "$CHILD_PID"
